...
import { ExcludeComment } from "./excludeComment/ExcludeComment";
...
export const CommentModal: React.FC<CommentModalProps> = ({
    ...
    // CORREÇÃO: Mudado de "" para false (booleano)
    const [isExcludeCommentModal, setExcludeCommentModal] = useState(false);
    // ADIÇÃO: Guarda o ID do comentário que será excluído
    ...
    return (
        ...
        >
                            ...
                            comments.map((comment) => (
                                ...
                                >
                                    <Box sx={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", width: "100%" }}>
                                        <Box sx={{ display: "flex", alignItems: "flex-start", flexGrow: 1, pr: 2 }}>
                                            <Avatar
                                                src={
                                                    comment.user?.avatar
                                        ...
                                        {/* CORREÇÃO: Agora abre o modal correto e salva o ID do comentário selecionado */}
                                        <IconButton
                                            size="small"
                                            onClick={() => {
                                                setSelectedCommentId(comment.id);
                                                setExcludeCommentModal(true);
                                            }}
                                            sx={{
            ...
            {/* MODAL DE EXCLUSÃO */}
            <ExcludeComment
                open={isExcludeCommentModal}
                onClose={() => {
                    setExcludeCommentModal(false);
                    setSelectedCommentId(null); // Limpa o ID ao fechar
                }}
                commentId={selectedCommentId} // Envia o ID para o componente interno
                onSuccess={fetchComments}    // Opcional: Se seu componente aceitar um callback para atualizar a lista após deletar
            />
        </Dialog>
    );
};